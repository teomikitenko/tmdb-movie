...
const Serials = () => {
  ...
  const searchVideo=()=>{
    ...
    if(!trailer){
      const openingCredits= data.videos.results.filter((vid) => vid.type === "Opening Credits")[0]
      if(!openingCredits){
        const teaser= data.videos.results.filter((vid) => vid.type === "Teaser")[0]
        return teaser
      }
            ...
            <div className="content">
              ...
              <div className="text_content">
                ...
                <div className="text_content_date">
                  ...
                  <Box component='span'  sx={{marginRight:'13px'}} className="genres">
                    {data.genres.map((genre, index) => {
                      if (index !== data.genres.length - 1) {
                        return <Typography variant="info_media" component='span' key={index}>{genre.name},</Typography>;
                      } else return <Typography variant="info_media" component='span' key={index}>{genre.name}</Typography>;
                    })}
                  </Box> 
                </div>

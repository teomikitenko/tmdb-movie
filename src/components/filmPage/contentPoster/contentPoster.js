...
console.log(videoUrl)
  ...
  const percent=data.vote_average>0? Math.round(data.vote_average * 10):null
       ...
       return {l:'#C60000',b:"#D55448"} 
        ...
        const takeCrew=()=>{
           const director=data.credits.crew.filter(job=>job.department === 'Directing')
         const writings=data.credits.crew.filter(job=>job.department === 'Writing')
...
<div className="content">
   <div className="content_img">
     <img className="img_title" src={base_img+data.poster_path} alt="poster"/>   
    </div>
   <div className="text_content">
       ...
       <div className="text_content_date">
           ...
           <span className="date"> {buildDate(data.release_date)} (UA)
            </span> 
            <span className="genres">{data.genres.map((genre,index)=><span key={index} >{genre.name},</span>)}</span> 
         <span className="runtime">{Math.trunc(data?.runtime/60)}г {data.runtime % 60}хв</span>
         </div>   
     ...
     {<div className="creator_bottom">
      ...
      {data&&crew? constructCrew(crew.dir,'Director').map((cr,index)=>{
       if(index<2){
         return(
           <li key={index}><p className="text_creator">{cr.name}</p>
         <p className="position">{cr.job}</p>
               ...
               :null  }
                 {data&&crew? constructCrew(crew.writ,'Writer').map((cr,index)=>{
                   if(index<3){
                     return(
                       <li key={index}><p className="text_creator">{cr.name}</p>
                     <p className="position">{cr.job}</p>
               ...
               :null  }
             {data&&crew? constructCrew(crew.writ,'Characters').map((cr,index)=>{
                   if(index<3){
                     return(
                       <li key={index}><p className="text_creator">{cr.name}</p>
                     <p className="position">{cr.job}</p>
   ...
   <Modal
        ...
        >
         <Box sx={style}>
          ...
          fontSize="small"
          sx={{cursor:'pointer',fill:'grey',position:'absolute',top:'9px',right:'12px'}}
          />
          <ReactPlayer
          width='984px'
          height='553px'
     ...
     {data.images.backdrops.map((media,index)=>{
      if(index<6){
        return(
          <SwiperSlide>
     <div class="container_for_img">
           <img loading="lazy" src={base_backdrop+media.file_path} alt=""/>
         </div>
     </SwiperSlide>
        ) 
      } return null
